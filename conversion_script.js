...
rimraf('photos', function () {
    fs.mkdirSync('photos')

    let exportFile = "photos/export.json";
    ...
    points.forEach(point => {
        let description = point.description ? point.description.cs : "";
        if (point.hasTreasure) {
            description += "<br/><br/>Zde můžete získat brožuru s mapou k pokladu na sbírání razítek v soutěži o ceny.";
        }
        var r = {
            "userId": "5a89a7d9a2297d0a4bf4e89c",
            "places": getPlaces(point),
            "title": point.title ? point.title.cs : "",
            ...
            "longitude": point.lng,
            "latitude": point.lat,
            "street1": point.address ? point.address.cs : "",
            "phone": point.tel ? point.tel.cs : "",
            "url": point.link ? point.link.cs : "",
            "email": point.email ? point.email.cs : ""
        }

        console.log(r);
        fs.appendFileSync(exportFile, ",\"" + uuidv1() + "\" : " + JSON.stringify(r));
        return r;
    });

    function copyFile(name, newname) {
        fs.createReadStream('src/assets/img/' + name).pipe(fs.createWriteStream('photos/' + newname + path.extname(name)));
    }

    function getPlaces(point) {
        let places = {};
        if (point.images && point.images[0]) {
            let id = uuidv1();
            copyFile(point.images[0], id);
            places[id] = {
                "key": id,
                "filename": id + "." + point.images[0].split('.')[1],
                "pointtype": 3,
                "region": [
                    "2"
                ]
